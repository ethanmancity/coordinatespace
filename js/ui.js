...
(function (global) {
  ...
  function sliderRangeFor(id) {
    if (id === 'cyl-phi' || id === 'sph-phi') return ANGLE_RANGES[S.getUnits()].phi;
    if (id === 'sph-theta') return ANGLE_RANGES[S.getUnits()].theta;
    ...
    SECTIONS.forEach(section => {
      for (const key in FIELDS[section]) {
        ...
        input.addEventListener('focus', () => {
          S.setActive(section);
          refreshActive();
          /* on touch devices the on-screen keyboard can cover the
             focused field — nudge it into view once the keyboard
             starts animating in */

/* ============================================================
   ...
   World orientation: right-handed, the app's Z axis is vertical
   (up/down), X and Y lie on the horizontal ground plane. three.js
   has Y up, so every app-space point (x, y, z) is mapped to the
   world position (y, z, x). The grid is drawn directly in the
...
(function (global) {
  ...
  /* Convert an app-space point (x, y, z) — Z vertical, X/Y horizontal —
     into a three.js world position (y, z, x). Arrays hold app
     coordinates; a Vector3 argument is already world space (cloned). */
  function v3(a, b, c) {
    if (Array.isArray(a)) return new T.Vector3(a[1], a[2], a[0]);
    if (a && a.isVector3) return a.clone();
    return new T.Vector3(b, c, a);
  }
  ...
  function buildRectOverlay(p) {
    const ax = p.z, ay = p.x, az = p.y; /* world → app */
  ...
  function buildCylOverlay(p) {
    ...
    if (Math.abs(az) > EPS) {
      ...
      zLab.position.set(ay, az / 2, ax);
      overlay.add(zLab);
    }
  }

  /* ---- Spherical mode (ρ, θ, φ): translucent shell, polar θ arc,
         azimuthal φ arc ---- */
  function buildSphOverlay(p) {
    ...
    const phi = Math.atan2(ay, ax);                   /* azimuth from +X */
    const theta = Math.acos(clamp(az / rho, -1, 1));  /* polar from +Z */

    /* polar arc: from the +Z axis down to the vector,
       in the vertical plane at azimuth φ */
    if (theta > 1e-4) {
      overlay.add(dashedArcTheta(rho, phi, theta, col, 0.9));
      const lab = makeLabel('θ', css(col), 0.42 * dashScale);
      const pm = theta / 2;
      lab.position.set(
        rho * Math.sin(pm) * Math.sin(phi) * 1.16,
        rho * Math.cos(pm) * 1.16,
        rho * Math.sin(pm) * Math.cos(phi) * 1.16);
      overlay.add(lab);
    }

    /* azimuthal arc in the XY plane, radius ρ·sin θ */
    const rPlane = rho * Math.sin(theta);
    if (rPlane > 1e-4 && Math.abs(phi) > 1e-4) {
      overlay.add(dashedArcXY(rPlane, 0, phi, col, 0.9));
      const lab = makeLabel('φ', css(col), 0.42 * dashScale);
      lab.position.set(
        Math.sin(phi / 2) * (rPlane + 0.24),
        0,
        Math.cos(phi / 2) * (rPlane + 0.24));
      overlay.add(lab);
    }

    const rhoLab = makeLabel('ρ', css(col), 0.42 * dashScale);
    rhoLab.position.set(ay * 0.5, az * 0.5, ax * 0.5);
    overlay.add(rhoLab);
  ...
  /* dashed circular arc in the ground plane (app XY), sweeping a0 → a1
     from the app +X axis (world +Z) toward app +Y (world +X): the
     app point (r·cos a, r·sin a, 0) becomes world (r·sin a, 0, r·cos a) */
  function dashedArcXY(radius, a0, a1, color, opacity) {
    ...
    for (let i = 0; i <= n; i++) {
      ...
      pts.push(new T.Vector3(Math.sin(a) * radius, 0, Math.cos(a) * radius));
    }
    const geo = new T.BufferGeometry().setFromPoints(pts);
    const mat = new T.LineDashedMaterial({
  ...
  /* dashed polar arc on a sphere of radius r: from the +Z axis
     (θ = 0) down to θ = polar, in the vertical plane at azimuth
     `azim`. App points (r sin t cos a, r sin t sin a, r cos t) become
     the world positions (r sin t sin a, r cos t, r sin t cos a). */
